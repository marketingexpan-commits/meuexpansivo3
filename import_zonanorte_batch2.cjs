...
function parseStudents(text) {
    const students = [];
    // Parsing strategy: regexGlobal finds the student block start.
    // Check window for Shift and Class.

    // Note: The data format might contain multi-line addresses.
    ...
    while ((match = regexGlobal.exec(plainContent)) !== null) {
        ...
        // Search window: match start + 600 chars
        ...
        // Find Class (Turma) - look for "A dd/mm/yyyy" or "B dd/mm/yyyy"
...
async function importStudents() {
    console.log("Reading Zona Norte Batch 2 data...");
    if (!fs.existsSync('zonanorte_batch2_data.txt')) {
        console.error("Data file not found!");
        return;
    }
    const rawText = fs.readFileSync('zonanorte_batch2_data.txt', 'utf8');
    ...
    for (const data of parsed) {
        ...
        // Using merge: true to avoid overwriting existing non-student fields
        batch.set(docRef, student, { merge: true });
        count++;
        total++;

        // Add grades for everyone except strictly 'Infantil' if we enable that logic, 
        // but User said "Mantenha a estrutura do Financeiro ativa para todos" 
        // and usually we implicitly add grades for non-Infantil.
