...
export function SidebarItem({ label, isActive = false, onClick, badge, collapsed = false, icon }: SidebarItemProps) {
    return (
        ...
        >
            {icon && (
                <div className={`w-5 h-5 flex items-center justify-center shrink-0 ${isActive ? "text-blue-950" : "text-slate-500 group-hover:text-slate-700"}`}>
                    {icon}
                </div>
            )}
            ...
            {!collapsed && badge !== undefined && badge > 0 && (
                <span className="ml-auto bg-orange-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                    {badge}
                </span>
            )}

            {!collapsed && !badge && (
                <ChevronRight className={`ml-auto w-4 h-4 text-slate-300 transition-opacity ${isActive ? "opacity-100 text-blue-950" : "opacity-0 group-hover:opacity-100"}`} />
            )}
