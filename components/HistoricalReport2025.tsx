...
import { Student, GradeEntry, SchoolUnit } from '../types';
...
export const HistoricalReport2025: React.FC<HistoricalReport2025Props> = ({ student, grades, unitData }) => {
    ...
    const historicalGrades = useMemo(() => {
        ...
        // Regra de Status Soberana: Se o aluno tem média global aprovada, todas as matérias são "Aprovado"
        // No banco, já forçamos isso na ingestão, mas garantimos aqui na consulta.
        const overallAverage = student2025Grades.reduce((acc, g) => acc + (g.mediaAnual || 0), 0) / student2025Grades.length;
        ...
        return hsSubjects2025.map(subject => {
            ...
            return {
                ...
                situacao: isOverallApproved ? 'Aprovado' : (grade.situacaoFinal || 'Reprovado')
            };
        }).filter(Boolean) as { subject: string; mediaAnual: number; situacao: string }[];
    }, [grades, student.id, hsSubjects2025]);
