...
export const MessageBox: React.FC<{ student: Student; onSendMessage: (message: Omit<SchoolMessage, 'id'>) => Promise<void>; unitContacts: UnitContact[]; teachers?: Teacher[]; onSubmitTicket?: (subject: string, message: string) => Promise<void>; }> = ({ student, onSendMessage, unitContacts, teachers = [], onSubmitTicket }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (recipient === MessageRecipient.DIRECTION) {
    ...
    setIsSending(true);
    try {
      // Modifica o conteúdo se for para um professor ou coordenador específico
      ...
      } else if (recipient === MessageRecipient.COORDINATION && selectedCoordinatorId) {
        const contact = unitContacts.find(c => c.id === selectedCoordinatorId);
        if (contact) {
          finalContent = `[PARA: ${contact.name}] ${content}`;
        }
      }
      ...
      <div className="bg-white p-6 rounded-lg shadow-md border border-gray-100">
        {isSent ? (
          <div className="flex flex-col items-center justify-center h-48 text-center bg-orange-50 rounded-lg border border-orange-200 animate-fade-in">
            ...
            {recipient === MessageRecipient.DIRECTION && (
