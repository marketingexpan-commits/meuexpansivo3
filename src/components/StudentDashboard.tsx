...
interface StudentDashboardProps {
    ...
    // Props extras omitidas para brevidade se não forem usadas direta ou explicitamente
    [key: string]: any;
}
...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ student, grades, teachers = [], attendanceRecords, unitContacts = [], onLogout, onSendMessage }) => {
    ...
    const absencesThisMonth = useMemo(() => {
        ...
        return studentAttendance.filter(record => {
            const recordDate = new Date(record.date); // Interprets YYYY-MM-DD as T00:00:00.000Z
            return recordDate.getUTCFullYear() === currentYearUTC &&
                recordDate.getUTCMonth() === currentMonthUTC &&
    ...
    const supportNeededGrades = studentGrades.filter(g => {
        ...
        const hasDifficulty = Object.values(g.bimesters).some((b: BimesterData) => b.difficultyTopic && b.difficultyTopic.trim().length > 5);
                ...
                <div className="mb-8 border-b-2 border-blue-950 pb-4">
                    <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                            <h4 className="text-xl font-bold text-gray-800 uppercase">{headerText}</h4>
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 border border-gray-300 text-sm">
                        <thead className="bg-blue-50 print:bg-gray-100">
                            <tr>
                                <th rowSpan={2} className="px-3 py-3 text-left font-bold text-gray-700 uppercase border-r border-gray-300 w-40">Disciplina</th>
                                {[1, 2, 3, 4].map(num => (
                    ...
                    <div className="mt-8 print:hidden">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {supportNeededGrades.map((grade) => {
                                ...
                                const difficulties = Object.entries(grade.bimesters)
                                    .map(([key, data]) => {
                            ...
                            <div className="p-6 overflow-y-auto flex-grow bg-gray-50/50">
                                {isLoadingAI ? (
                                    ...
                                    <div className="prose prose-slate max-w-none">
                                        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 text-gray-700 leading-relaxed whitespace-pre-wrap"
                                            dangerouslySetInnerHTML={{ __html: modalContent.tip.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br/>') }}
                                        />
                                    </div>
                                )}
                            </div>
...
const MessageBox: React.FC<{ student: Student; onSendMessage: (message: Omit<SchoolMessage, 'id'>) => Promise<void>; unitContacts: UnitContact[] }> = ({ student, onSendMessage, unitContacts }) => {
    ...
    const mySegment = getStudentSegment(student.gradeLevel);

    const relevantCoordinators = unitContacts.filter(c => {
        if (c.unit !== student.unit) return false;
        if (!c.segment || c.segment === CoordinationSegment.GERAL) return true;
        return c.segment === mySegment;
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await onSendMessage({
                ...
                unit: student.unit,
                recipient,
                // Se for coordenação, adiciona o nome do coordenador específico no conteúdo ou metadados
                // Como a interface SchoolMessage não tem campo 'coordinatorId', vamos prefixar no conteúdo
                content: recipient === MessageRecipient.COORDINATION && selectedCoordinator
                    ? `[PARA: ${selectedCoordinator}] ${content}`
                    : content,
                messageType,
            ...
            // Erro já é tratado no App.tsx com um alerta
        } finally {
            setIsSending(false);
        }
    };
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="recipient" value={recipient} onChange={e => { setRecipient(e.target.value as MessageRecipient); setSelectedCoordinator(''); }} className="w-full p-2 border border-gray-300 rounded-md shadow-sm bg-white">
                                    {Object.values(MessageRecipient).map(r => <option key={r} value={r}>{r}</option>)}
                                </select>

                                {recipient === MessageRecipient.COORDINATION && (
                                    <div className="mt-2 animate-fade-in">
                                        <label className="block text-xs font-bold text-blue-800 mb-1">Selecione o Coordenador:</label>
                                        <select
                                            value={selectedCoordinator}
                                            onChange={e => setSelectedCoordinator(e.target.value)}
                                            className="w-full p-2 border border-blue-300 rounded-md bg-blue-50 text-sm"
                                            required
                                        >
                                            <option value="">-- Qualquer Disponível --</option>
                                            {relevantCoordinators.map(c => (
