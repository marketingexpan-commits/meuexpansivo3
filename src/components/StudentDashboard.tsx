...
import { Student, GradeEntry, Teacher, Subject, SchoolMessage, MessageRecipient, MessageType, AttendanceRecord, AttendanceStatus, BimesterData } from '../types'; 
...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({ student, grades, teachers = [], attendanceRecords, onLogout, onSendMessage }) => {
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen print:p-0 print:bg-white"> 
      <div className="max-w-7xl mx-auto bg-white shadow-lg rounded-lg p-6 print:shadow-none print:w-full">
                        ...
                        <tr key={grade.id} className="hover:bg-gray-50 transition-colors border-b border-gray-300">
                            ...
                            <td className="px-1 py-2 text-center align-middle">
                                <span className={`inline-block w-full py-0.5 rounded text-[9px] uppercase font-bold border ${
                                    grade.situacaoFinal === 'Aprovado' ? 'bg-green-50 text-green-700 border-green-200' : 
        ...
        <MessageBox student={student} onSendMessage={onSendMessage} />
...
const MessageBox: React.FC<{ student: Student; onSendMessage: (message: Omit<SchoolMessage, 'id'>) => Promise<void>; }> = ({ student, onSendMessage }) => {
