...
async function importStudents() {
    console.log("Reading Zona Norte Batch 4 data...");
    if (!fs.existsSync('zonanorte_batch4_data.txt')) {
        console.error("Data file not found!");
        return;
    }
    const rawText = fs.readFileSync('zonanorte_batch4_data.txt', 'utf8');
    ...
    if (count > 0) {
        console.log(`Committing final batch of ${count} operations...`);
        await batch.commit();
    }

    console.log(`Import Complete. Total students processed: ${total}`);
}
