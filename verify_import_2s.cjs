...
async function verify() {
    ...
    console.log("Verifying students (2nd Grade)...");
    ...
    for (const id of idsToCheck) {
        ...
        if (!doc.exists) {
            ...
            console.log(`   Grade: ${data.gradeLevel} (Expected: 2ª Série - Ens. Médio)`);
            console.log(`   Unit: ${data.unit} (Expected: Extremoz)`);
            console.log(`   Shift: ${data.shift} (Expected: Matutino)`);
            console.log(`   Class: ${data.schoolClass} (Expected: 03)`);

            if (data.gradeLevel === '2ª Série - Ens. Médio' &&
                data.unit === 'Extremoz' &&
                data.shift === 'Matutino' &&
                data.schoolClass === '03') {
